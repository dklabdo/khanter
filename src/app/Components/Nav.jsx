...
function Nav() {
  ...
  return (
    <div className="w-full top-0 bg-white/70 backdrop-blur-3xl z-30 max-w-screen m-0 p-0 absolute left-0 right-0   flex flex-col     ">
      <div className="w-full   flex items-center px-2 md:px-5 justify-between  h-14    ">
        <div>
          {" "}
          <Image src="/logo.svg" width={110} height={40} alt="...." />{" "}
        </div>
        <ul className="  text-black py-2  px-16 rounded-xl 
        ...
        <div className="items-center flex lg:hidden gap-1 ">
          <button
            ...
            className="text-sm scale-90 text-black  transition-all cursor-pointer font-light rounded-md  px-2 py-2  flex-row-reverse flex gap-1 items-center  "
          >
            {" "}
            {show ? <X size={25} /> : <MenuIcon size={25} />}
          </button>
        </div>
        <button className="px-4 hidden lg:flex scale-[.8] cursor-pointer hover:bg-black  items-center gap-2  text-sm text-white  bg-primary rounded-lg  py-3  " >
